...
class AddEmployeeWindow(QMainWindow):
    ...
    def get_employee_data(self):
        ...
        elementary_name = self.ui.elementary_school.toPlainText()
        ...
        highschool_fin = self.ui.yeargraduate_highschool.toPlainText()

        college_name = self.ui.College.toPlainText()
        ...
        gradschool_name = self.ui.Graduateschool.toPlainText()
        ...
        def get_school_id(school_name, school_address):
            """Fetch or insert a school and return its School_Id."""
            conn = sqlite3.connect(get_database_path())
            cursor = conn.cursor()

            # Check if the school exists
            cursor.execute("SELECT School_Id FROM School WHERE Name = ? AND Address = ?", (school_name, school_address))
            ...
            cursor.execute("INSERT INTO School (Name, Address) VALUES (?, ?)", (school_name, school_address))
            conn.commit()
            school_id = cursor.lastrowid
            conn.close()
            return school_id

        # Fetch or create School IDs for each provided school
        elementary_id = get_school_id(elementary_name, elementary_address) if elementary_name and elementary_address else None
        highschool_id = get_school_id(highschool_name, highschool_address) if highschool_name and highschool_address else None
        college_id = get_school_id(college_name, college_address) if college_name and college_address else None
        gradschool_id = get_school_id(gradschool_name, gradschool_address) if gradschool_name and gradschool_address else None

        # Only proceed if at least one school ID or relevant field is filled
        if elementary_id or highschool_id or college_id or gradschool_id:
